...
for Make("f").val in Array(CONDA_PREFIX.val) + \
        Array("/pkgs/cudatoolkit-dev-9.2-0/bin"):
    subprocess.call(["ln",
                     "-s",
                     str(f.val),
                     str(CONDA_PREFIX.val) + "/bin/" + str(f.val)],
                    shell=True)
_rc0 = subprocess.call(["set", "CUDA_HOME=" +
                        str(CONDA_PREFIX.val) +
                        "/pkgs/cudatoolkit-dev-9.2-0"], shell=True)
for Make("f").val in Array(CONDA_PREFIX.val) + \
        Array("/pkgs/cudatoolkit-dev-9.2-0/lib64"):
    subprocess.call(["ln",
                     "-s",
                     str(f.val),
                     str(CONDA_PREFIX.val) + "/lib/" + str(f.val)],
                    shell=True)
